describe("Handle TicketsNew", () => {
  it("Login into the site", () => {
    ...
    cy.wait(3000);
    cy.get(
      ":nth-child(3) > .H5RunningMovieV2_runningMovie__5uA8A > a > .H5RunningMovieV2_imgWrapper__Tadcr > .bgImg"
    )
      .should("be.visible")
      .click();
    ...
    cy.get(
      ":nth-child(1) .MovieSessionsListing_col2__4GGXs > :nth-child(1)"
    ).click();

    
    cy.get(':nth-child(1) .FixedSeating_seatL__dETyY > :nth-child(3) >.available')
 /*.each(($seat) => {
    cy.wrap($seatq1).click();
  });*/
